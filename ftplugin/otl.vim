...
" To support asciidoc
" Xuesong Lu
" Keyword: otl, asciidoc, outliner, otl_asciidoc_style 
" 2011-05-28 Sat
" 2011-06-04 Sat
" reference: asciidoc.vim from Stuart
" in .vimrc, set following:
" au BufNewFile,BufRead,BufEnter *.txt	let g:otl_asciidoc_style = 1
" au BufNewFile,BufRead,BufEnter *.txt	setf otl
"
" 2011-06-30 Thu
" Implemented one line mode for
"  1) ,>, ,<
"  2) ,u, ,d
"
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""" 
...
" Set the named buffer var to the global var of the same name if it exists,
" else to default
function! s:OtlDefaultGlobalVar(name, default)
  if !exists('g:' . a:name)
...
" set g:otl_install_toolbar to 0 if you don't want Outliner toolbar buttons
call s:OtlDefaultGlobalVar("otl_install_toolbar", 1)
...
" set g:no_otl_maps to 1 if you don't want TVO mappings installed
call s:OtlDefaultGlobalVar("no_otl_maps", 0)

" set g:otl_map_tabs to 1 if you want TVO to map <Tab> and <S-Tab> to demote and promote
call s:OtlDefaultGlobalVar("otl_map_tabs", 0)
...
if exists('*VikiMinorMode')
  ...
  " set g:otl_viki_name_types to the default Viki name types that you want to
  " use (see viki.txt)
  " s Simple viki name
  " c CamelCase
  ...
  " i interviki
  " u URL
  call s:OtlDefaultGlobalVar("otl_viki_name_types", "'csSeui'")
...
if g:otl_use_viki
  ...
  "
  " if state is +/-1
  "   sets b:vikiCommentStart/End
  "     built from &commentstring
  " can also set
  "   b:vikiNameTypes
  "   b:vikiSplit
  "   b:vikiAnchorMarker
...
if !exists("s:otl_loaded_functions")
  ...
  if ((exists("g:otl_asciidoc_style")) && (g:otl_asciidoc_style ))
    let s:tabsAtBeginningOfLine = '\_^=\+\s\+'
    " let s:beginningOfHeaderLine = s:tabsAtBeginningOfLine . '\ze[^=]'
    let s:beginningOfHeaderLine = s:tabsAtBeginningOfLine . '\ze.*'
    " let s:tabsAtBeginningOfLine = '\_^=*\s+'
    " let s:beginningOfHeaderLine = s:tabsAtBeginningOfLine . '\ze*'
    let s:beginningOfTextLine = '^[^=]'
    " s:beginningOfAnyLine is NOT to be used with otl_asciidoc_style
    let s:beginningOfAnyLine = s:tabsAtBeginningOfLine . '\%(|\s*\ze\|\ze[^|\t]\)'  "non-content prefix
    " s:textWithMarkers is NOT to be used with otl_asciidoc_style
  ...
  function OtlFoldLevel(n)
    ...
    " otl_asciidoc_style
    if ((exists("g:otl_asciidoc_style")) && (g:otl_asciidoc_style))
      if !exists("s:current_heading")
        s:current_heading = 10
      endif

      " to match two line heading
      " =-~^+
      let nextLine = getline(a:n + 1)
      if ((nextLine=~'^=\{2,}$') && (len(nextLine) == len(thisLine)))
        let s:current_heading = 1
        return '>1'
      elseif ((nextLine=~'^-\{2,}$') && (len(nextLine) == len(thisLine)))
        let s:current_heading = 2
        return '>2'
      elseif ((nextLine=~'^\~\{2,}$') && (len(nextLine) == len(thisLine)))
        let s:current_heading = 3
        return '>3'
      elseif ((nextLine=~'^^\{2,}$') && (len(nextLine) == len(thisLine)))
        let s:current_heading = 4
        return '>4'
      elseif ((nextLine=~'^+\{2,}$') && (len(nextLine) == len(thisLine)))
        let s:current_heading = 5
        return '>5'
      endif
      ...
      elseif (thisLine=~'^==')
        let s:current_heading = 2
        return '>2'
      elseif (thisLine=~'^=') " title
        let s:current_heading = 1
        return '>1'
      endif

      if (thisLine=~'^<<<') " page break, better to set same fold level as 
        return '>1'
      elseif (thisLine=~'^TODO' ||thisLine=~'^Note' || 
        \ thisLine=~'^Important' || thisLine=~'^Warning' || thisLine=~'^Caution')
        return '>' . (s:current_heading + 1)
      elseif (thisLine=~'^\.')
        if (thisLine=~'^\.\{4}')
          return '>' . (s:current_heading + 4)
        elseif (thisLine=~'^\.\{3}')
          return '>' . (s:current_heading + 3)
        elseif (thisLine=~'^\.\{2}')
          return '>' . (s:current_heading + 2)
        elseif (thisLine=~'^\.\{1}')
          return '>' . (s:current_heading + 1)
        endif
      else
    ...
    else " end of otl_asciidoc_style
  ...
  function s:OtlSavePos()
    normal! mp
    let b:otlSaveHlsearch = &hlsearch
  endfunction

  function s:OtlRestorePos()
    normal! `p
    let &hlsearch = b:otlSaveHlsearch
  endfunction
  ...
  function s:OtlJumpLev(direction)
    ...
    " =-~^+
      ...
      if ((nextLine=~'^=\{2,}$') && (len(nextLine) == len(thisLine)))
        let two_line_heading = 1
        let two_line_start = 1
      elseif ((nextLine=~'^-\{2,}$') && (len(nextLine) == len(thisLine)))
        let two_line_heading = 2
        let two_line_start = 1
      elseif ((nextLine=~'^\~\{2,}$') && (len(nextLine) == len(thisLine)))
        let two_line_heading = 3
        let two_line_start = 1
      elseif ((nextLine=~'^^\{2,}$') && (len(nextLine) == len(thisLine)))
        let two_line_heading = 4
        let two_line_start = 1
      elseif ((nextLine=~'^+\{2,}$') && (len(nextLine) == len(thisLine)))
        let two_line_heading = 5
        let two_line_start = 1
      else
        let two_line_heading = 10
        let two_line_start = 0
      endif

      while((two_line_start == 1) && (line > 0 && line < line('$')))
        ...
        if ((nextLine=~'^=\{2,}$') && (len(nextLine) == len(thisLine)) && (two_line_heading == 1))
            return line
        elseif ((nextLine=~'^-\{2,}$') && (len(nextLine) == len(thisLine)) && (two_line_heading == 2))
            return line
        elseif ((nextLine=~'^\~\{2,}$') && (len(nextLine) == len(thisLine)) && (two_line_heading == 3))
            return line
        elseif ((nextLine=~'^^\{2,}$') && (len(nextLine) == len(thisLine)) && (two_line_heading == 4))
            return line
        elseif ((nextLine=~'^+\{2,}$') && (len(nextLine) == len(thisLine)) && (two_line_heading == 5))
            return line
        endif
    ...
    while(line > 0 && line < line('$'))
      ...
      if ((exists("g:otl_asciidoc_style")) && (g:otl_asciidoc_style ))
      " handle otl_asciidoc_style
        let llev = matchend(getline(line), s:beginningOfHeaderLine)
        if (llev == lev)
          return line
        endif
      else
  ...
  function s:OtlPromoteBy(levels)
    ...
    " handle two lines mode
    " to find the end line of current fold
    let lineFoldEnd = s:OtlJumpLev(1)
    if (lineFoldEnd < 0)
      let lineFoldEnd = line('.')
    else
      let lineFoldEnd = s:OtlJumpLev(1) - 1
    endif
    ...
    " =-~^+
    let line = line('.')
    while((line < line('$') && line < lineFoldEnd))
      ...
      if ((nextLine=~'^=\{2,}$') && (len(nextLine) == len(thisLine)))
        ...
        while(i < len(nextLine))
          let substLine = substLine . '-'
          let i = i + 1
        endwhile
        call setline(line + 1, substLine)
...
"    setlocal noexpandtab
    ...
    let b:undo_ftplugin = "setlocal foldtext< foldmethod< foldexpr< formatoptions< comments< ai< et< foldcolumn< ts< sw<"
      ...
      \ . " b:vikiCommentEnd b:vikiSplit b:otl_installed_mappings "
      \ . "| mapclear <buffer>"

    " changes to global settings that must be restored
    "
    let b:otlStsave=&sta
    set nosta

    let b:otlFcsave=&fillchars
    set fillchars-=fold:-
    set fillchars+=fold:\ " space

    " Deal with old format of 'backspace'
    let b:otlBssave=&backspace
    if b:otlBssave == "0"
      set backspace=""
    elseif b:otlBssave == "1" || b:otlBssave == "2"
      set backspace=eol
    endif
    set backspace+=indent
    set backspace+=start

    let b:otlMmsave=&mousemodel
  ...
  function s:OtlExitBuffer()
    let &backspace=b:otlBssave
    let &fillchars=b:otlFcsave
    let &sta=b:otlStsave
    let &mousemodel=b:otlMmsave
  endfunction
